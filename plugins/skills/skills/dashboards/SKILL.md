...
| `dashboard_clone`  | Copy a dashboard into a workspace, cloning + remapping its variables/scopes |
...
- `dataStream.id` and `dataStream.name` - feed straight into `data_stream_query` (together with the dashboard's `workspaceId`, which `data_stream_query` requires) or use `tile_query` to skip the assembly.
...
## Pattern: "duplicate / build me a dashboard like X"

- **Faithful copy (same or different workspace):** `dashboard_clone({ dashboardId, sourceWorkspaceId, targetWorkspaceId })`. It clones the dashboard's variables and scopes into the target workspace and remaps tile references, so the copy is self-contained. Prefer this over `dashboard_get` + `dashboard_create`, which only copies `content` and leaves tiles pointing at the source workspace's variables/scopes.
- **Copy then tweak ("like X but for Y"):** `dashboard_clone` first, then `dashboard_get` the clone, adjust, and `dashboard_update` - so the tweaks sit on top of the already-remapped variables/scopes.
- For small additions, adjust individual tiles with `tile_create` / `tile_update` rather than hand-crafting the full content blob.
...
- The user wants to share a dashboard via a public link → `sharing`.
- The user wants to control who can view or edit a dashboard → `permissions`.
